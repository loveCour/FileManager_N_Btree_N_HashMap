...
SimpleIndex<keyType>::SimpleIndex(int maxKeys, int unique)
	: NumKeys(0), Keys(nullptr), RecAddrs(nullptr)
{
	Init(maxKeys, unique);
}
...
{//to do 이거 왜그런지 알아보자
	...
	if(RecAddrs != nullptr) delete RecAddrs;
}
...
template <class keyType>
int SimpleIndex<keyType>::Search(const keyType key, const int recAddr, const int exact) const
{
...
{
	stream << "SimpleIndex MaxKeys: " << MaxKeys << "\tNumKeys: " << NumKeys << endl;
	for (int i = 0; i < NumKeys; i++)
		stream << "\tKey[" << i << "] " << Keys[i] << " RecAddr: " << RecAddrs[i] << endl;
}
